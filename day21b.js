...
const rotate = (direction, nSteps, str) => {
  ...
  let rotationIdx = direction === 'right' ?
    nSteps :
    (nSteps * -1);
...
const originalRotateBasedOn = (x, str) => {
  const idx = str.indexOf(x);
  const inc = idx > 3 ? 2 : 1;
  return rotate('left', idx + inc, str);
};
...
const rotateBasedOn = (x, str) => {
  ...
  let i = 2;
  do {
    result = rotate('right', i++, str);
  } while (originalRotateBasedOn(x, result).join('') !== str.join(''));
  return result;
};
